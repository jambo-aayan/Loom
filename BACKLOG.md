...
- **Additional strategies beyond the first.** Only one concrete strategy (low-volatility large-cap/index, small consistent wins) is being specced for v1, to prove the pipeline end-to-end. The `Strategy` interface is designed to support many; actually building more is future ticket work.
- **Candidate strategy #2 — "volatility harvesting" around a held position.** Instead of buying and holding to one fixed target, spot an asset that looks temporarily undervalued relative to its own recent range (pullback %, RSI/z-score vs a rolling mean), buy it, then keep working the position while held: trim on a bounce back toward the mean or a gain vs *your* cost basis, optionally add on further weakness (bounded by a max position size / risk limit). This needs no new interface — `generate_signals(market_data, positions, account)` already takes current positions as an input, so a strategy can already emit both entry and position-management signals for a name it's already holding. Two things to weigh when this gets specced: (1) spread/stamp-duty costs need a minimum-move threshold or frequent round-trips just bleed to costs; (2) the ISA wrapper has no CGT, so unlike a normal account, frequent buy/sell on the same name doesn't carry a tax drag — a genuine fit for this style. Build after the first strategy proves the pipeline.
