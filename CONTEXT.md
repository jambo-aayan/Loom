...
**Strategy**
A pluggable component that generates `Signal`s from market data, current positions, and account state. Each one is independently identifiable (every `Signal` carries the `strategy_id` of the `Strategy` that produced it) and independently evaluable (performance is tracked per strategy via its own `Book`, not just in aggregate). The system was never meant to have only one.

A `Strategy` is a named configuration, not a piece of code: several `Strategy`s may be configurations of one underlying implementation, differing only in their `Strategy config version`. Two `Strategy`s that buy dips at different depths, on different universes, are two `Strategy`s — each with its own `Book` and its own record — however much code they share. The unit of measurement is the `Strategy`, and merging two into one would merge their P&L and destroy the comparison a `Book` exists to make. See `docs/adr/0023-strategy-structure-segments-and-shared-implementations.md`.
